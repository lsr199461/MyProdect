...
func(tx *Transaction)Sign(privateKey ecdsa.PrivateKey,prevTXs map[string]Transaction){
	...
	for inID,vin:=range txCopy.Vin{
		...
		txCopy.ID=txCopy.Hash() //拷贝哈希拷贝公钥
		...
		tx.Vin[inID].Signature=signature//赋值签名
...
//钱包处理交易
func  NewUTXOTransaction(wallet *Wallet,to string,amout int,UTXOSet *UTXOSet)*Transaction{
	var inputs []TXInput  //输入
	var outputs []TXOutput //输出

	pubkeyhash:=HashPubkey(wallet.PublicKey)//获取公钥
	acc,validOutputs:=UTXOSet.FindSpendableOutpus(pubkeyhash,amout)
	if acc<amout{
		log.Panic("你的钱不够，转你麻痹转转转")
	}
	...
	tx.ID=tx.Hash()
	UTXOSet.blockchain.SignTransaction(&tx,wallet.PrivateKey)//签名
	return &tx
}
